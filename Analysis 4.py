...
If a student is influenced, then what matters more: confidence or projects?
Is this different for STEM and non STEM students?

Conclusion:
1. When a student is influenced, percentage of students undertaking projects and
having high self-confidence is similar.
2. When a student is influenced, the percentage of STEM students and non-STEM students
with high self-confidence is similar.
3. When a student is influenced, percentage of STEM students undertaking individual projects is slightly
...
# c) Relation with confidence for STEM students
very_influenced_confidence_stem = \
    data[(data['Influenced'] == 1) & (data["is_stem"] == 1)].groupby(['SelfConfidence']).count()['Age']
array_1 = [1, 2, 3, 4, 5]
array_2 = [very_influenced_confidence_stem[x] for x in array_1]
answer_confidence = (array_2[3] + array_2[4]) * 100 / sum(array_2)

# d) Relation with confidence for non-STEM students
very_influenced_confidence_non_stem = \
    data[(data['Influenced'] == 1) & (data["is_stem"] == 0)].groupby(['SelfConfidence']).count()['Age']
...
# e) Relation with individual projects for STEM students
very_influenced_project_stem = \
    data[(data['Influenced'] == 1) & (data["is_stem"] == 1)].groupby(['IndividualProject']).count()['Age']
array_1_p = [0, 1]
array_2_p = [very_influenced_project_stem[x] for x in array_1_p]
answer = array_2_p[1] * 100 / sum(array_2_p)

# f) Relation with individual projects for non-STEM students
very_influenced_projects_non_stem = \
    data[(data['Influenced'] == 1) & (data["is_stem"] == 0)].groupby(['IndividualProject']).count()['Age']
